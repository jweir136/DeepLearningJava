...
/** Creates a very simple perceptron ANN constiting of only 1 input layer, and 1 output layer containing a single node. Can only be used for binary classification. Uses a penality on the weights in order to limit overfitting
*/
public class BinaryPerceptronPenalty implements Classifier {
  private double[][] X;
  private int[] y;
   ...
   *  @param lambda:
   *    The penality term. Dictates how much to limit the weights during training.
   *  @Precondition:
  ...
  */
  public BinaryPerceptronPenalty(int epochs, double learningRate, double lambda) {
    this.epochs = epochs;
  ...
  */
  public double error(double[][] X, int[] y) {
    ...
    for (int j = 0; j < this.weights.length; j++)
      sum += Math.pow(this.weights[j], 2);
    
    return error + this.lambda * sum;
  }
  ...
  */
  private double weightDeriv(int index) {
    ...
    for (int i = 0; i < this.X.length; i++)
      result += ((this.y[i] - predictions[i]) * (-this.X[i][index])) + (2.0 * this.lambda) * (this.weights[index]);
